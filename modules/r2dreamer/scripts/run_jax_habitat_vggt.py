...
def _flatten_vggt(out: dict) -> np.ndarray:
    ...
    Args:
        out: {"world_points": (37,37,3), "camera_pose": (9,)}
...
def main():
    ...
    buffer = VGGTReplayBuffer(config.buffer_capacity, VGGT_FEATURE_DIM)
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        obs = env.reset()
        extractor.reset()
        vggt_out = extractor.extract(obs["image"])
        features = _flatten_vggt(vggt_out)

        for i in range(config.prefill_steps):
            action = np.random.randint(0, config.num_actions)
            next_obs = env.step(action)
            next_vggt = extractor.extract(next_obs["image"])
            next_features = _flatten_vggt(next_vggt)

            success = next_obs.get("success", 0.0) > 0
            buffer.add(features, action, next_obs["reward"], next_obs["done"],
                       terminal=success)
        ...
        for step in range(config.total_steps):
            ...
            obs_dict = {"features": features, "is_first": obs.get("is_first", False)}
