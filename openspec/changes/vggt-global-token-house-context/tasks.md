...
## 2. Reducer: camera-branch-free PointNet max-pool

- [ ] 2.1 Implement a patch-only reducer: shared per-token MLP (`Dense → RMSNorm → SiLU`) over the 1369 patch tokens
- [ ] 2.2 Apply a single max-pool over the token axis (no mean branch), then `Dense(1024)` projection → `(…, 1024)`
- [ ] 2.3 Remove the camera side branch and the `concat → 2048`; assert output width is 1024, not 2048
- [ ] 2.4 Unit-check permutation invariance: shuffling the 1369 tokens leaves the `(…, 1024)` embedding unchanged
...
- [ ] 4.2 Wire the obs key so the `(1, 1024)` embedding is concatenated with the other obs embeddings for `R2RSSM` (no RSSM change)
- [ ] 4.3 Confirm the RSSM contract is untouched: `deter=2048`, `stoch=32×16`, `feat=2560`

## 5. Verification

- [ ] 5.1 Shape smoke test end-to-end: frame → global tokens → reducer → `(1, 1024)` → RSSM `observe` runs
...
- [ ] 6.2 Ablation stub only: note the camera-token A/B (`(1,1024)` vs `2048`)
