...
- [VGGT JAX Eviction-Onset Recompile Cliff](methods/vggt-jax-eviction-recompile.md) — Past frame ~36 JAX recompiles every frame (~43s each); `current_budgets_static` tuple drifts post-eviction. Prior 1.9× claim only holds pre-cap; n=100 median is 24.5s
...
Daily distillates from Claude Code sessions, written by the SessionEnd hook. See [`lessons/`](lessons/).
