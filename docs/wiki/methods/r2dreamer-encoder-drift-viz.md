...
| **Interactive 4-panel notebook** (Plotly 3D cloud + trajectory + slider + heatmaps) | **deferred** |
| MLP probe escalation | deferred (Plan B if notebook story is unclear) |
| Replication on a second pair | deferred |

The interactive notebook (Task #6 in the implementation plan) was deferred in favour of locking in the analytical narrative first. If the static figures already make the drift story clear, the notebook becomes optional polish; if not, it gets built next.
