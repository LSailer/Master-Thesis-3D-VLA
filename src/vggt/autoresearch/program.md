...
4. **Cache the PyTorch baseline**: run `uv run python -m src.vggt.autoresearch.bench_fast --setup` ONCE. This caches the PyTorch baseline timing so every iteration measures only JAX.
5. **Initialize results.tsv**: create `src/vggt/autoresearch/results.tsv` with this tab-separated header (8 columns):
   ```
...
Each experiment runs on a single GPU. The fitness metric is `speedup = pt_baseline_ms / jax_median_ms` at n=50 frames. Higher speedup is better. Target: speedup >= 1.0 means JAX beats the PyTorch baseline. You launch a run simply as: `uv run python -m src.vggt.autoresearch.bench_fast`.

**What you CAN do — modify exactly these 6 files:**
- `src/vggt/jax/attention.py`
- `src/vggt/jax/block.py`
- `src/vggt/jax/aggregator.py`
- `src/vggt/jax/backbone.py`
- `src/vggt/jax/rope.py`
- `src/vggt/jax/feature_extractor.py`
...
**What you CANNOT do:**
- Modify `src/vggt/jax/weight_transfer.py`. Changing this breaks parity silently — the weight-loading invariant must be untouchable.
- Modify anything under `tests/vggt/`. The parity tests are the referee; they cannot rewrite the rules they are graded by.
- Modify `src/vggt/jax/benchmark_streaming.py`. It defines the metric for guardrail sweeps.
- Modify `src/vggt/autoresearch/bench_fast.py`. It defines the fitness metric.
- Modify `src/vggt/autoresearch/program.md` (this file).
...
When an experiment is done, append a row to `src/vggt/autoresearch/results.tsv` (tab-separated, NOT comma-separated).
...
1. **Inspect state.** Look at git state: current branch, last commit. Run `tail -5 src/vggt/autoresearch/results.tsv` to see recent experiments.
2. **Propose one idea.** Pick ONE experimental idea. Edit 1-3 files in the CAN list. Keep the diff small.
3. **Fast parity gate.**
   ```
   uv run pytest tests/vggt/test_jax_parity.py::TestLevel1WeightTransfer tests/vggt/test_jax_parity.py::TestLevel2SharedBlockParity -q
   ```
   If fail: `git restore src/vggt/jax/`, append a `crash_parity` row, return to step 1.
4. **Bench.**
   ```
   uv run python -m src.vggt.autoresearch.bench_fast > run.log 2>&1
   ```
5. **Parse.**
   ```
   grep "^speedup:\|^jax_median_ms:\|^peak_mem_mb:" run.log
   ```
   If the grep is empty, the run crashed. Run `tail -n 50 run.log`, attempt ONE fix, re-run. If still broken: `git restore src/vggt/jax/`, append a `crash_bench` row, return to step 1.
6. **Compare vs best.** Find the best `speedup` value (column 4) across all `keep` rows in `results.tsv` so far.
   - If `new_speedup <= best_speedup * 1.02`: `git restore src/vggt/jax/`, append a `discard` row, return to step 1.
   - Else continue.
7. **Full parity gate.**
   ```
   uv run pytest tests/vggt/ -q
   ```
   If fail: `git restore src/vggt/jax/`, append a `crash_full` row, return to step 1.
...
```
uv run python -m src.vggt.jax.benchmark_streaming --seq-lens 10 50 100 --backends jax > guardrail.log
```
