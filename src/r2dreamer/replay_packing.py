"""Replay-batch packing: transition windows -> raw training-aligned arrays.

Provides ``replay_batch_to_arrays``, which turns the ``(B, T)`` grid of
``ReplayTransition`` objects returned by ``ReplayBuffer.sample`` (or an
already-packed ``ReplayBatch``) into a dict of raw JAX arrays keyed by
``obs``, ``actions``, ``rewards``, ``is_first``, and ``is_episode_end``.
"""
