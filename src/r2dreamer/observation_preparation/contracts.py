...
def encoder_module_kwargs_from_config(
    ...
    Thin re-export of the single dispatch source of truth in
    ``encoders/registry.py`` (subclass-aware via an MRO walk — see
    ``registry.resolve_registry_entry``). Kept here because callers outside
    ``agent.py`` (``module_factory.py``, ``checkpointing.py``,
    ``launch/train.py``) import it from this module.
    ...
    """
    # Imported lazily: encoders/registry.py imports normalize_encoder_module_kwargs
    # from this module, so a top-level import here would cycle.
    from src.r2dreamer.encoders.registry import make_encoder_kwargs

    return make_encoder_kwargs(config, encoder_module_cls)
