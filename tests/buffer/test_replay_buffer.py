"""Behavior checks for the scratch replay buffer implementation."""
...
class TestReplayBufferUnitBehavior:
    ...
    def test_add_stores_deepcopy_of_transition(self) -> None:
        ...
        stored = buffer.sample(batch_size=1, seq_len=1)[0][0]
        assert stored.obs[0] == 1.0

    def test_sample_returns_deepcopy_not_storage_reference(self) -> None:
        """Mutating a sampled transition must not mutate replay storage."""
        buffer = ReplayBuffer(capacity=2)
        buffer.add(_transition(1))

        sampled = buffer.sample(batch_size=1, seq_len=1)[0][0]
        sampled.obs[0] = 99.0

        sampled_again = buffer.sample(batch_size=1, seq_len=1)[0][0]
    ...
    def test_wraparound_sampling_keeps_chronological_sequences(self) -> None:
        ...
        np.random.seed(0)
        for _ in range(50):
            batch = buffer.sample(batch_size=4, seq_len=3)
    ...
    def test_sample_marks_window_start_as_sequence_start(
        ...
        sequence = buffer.sample(batch_size=1, seq_len=2)[0]
