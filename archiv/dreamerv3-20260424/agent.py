...
from .networks import Encoder, Decoder, RSSM, MLP, Actor, Critic, TwoHotDist, symlog
...
class DreamerAgent:
    def __init__(self, config: DreamerConfig, rng_key: jax.Array):
        ...
        self.reward_pred = MLP(config.mlp_hidden, config.mlp_layers, config.twohot_bins, name="reward")
        ...
        self.critic = Critic(config.mlp_hidden, config.mlp_layers, config.twohot_bins)
        self.twohot = TwoHotDist(config.twohot_bins)
        ...
        self._retnorm_mean = jnp.zeros(())
        self._retnorm_var = jnp.ones(())
    ...
    def train_step(self, batch: dict, rng_key: jax.Array) -> dict:
        (self.wm_state, self.actor_state, self.critic_state,
         self.critic_target_params, retnorm_mean, retnorm_var,
         metrics) = self._train_step_jit(
            self.wm_state, self.actor_state, self.critic_state,
            self.critic_target_params, self._retnorm_mean, self._retnorm_var,
            batch, rng_key,
        )
        self._retnorm_mean = retnorm_mean
        self._retnorm_var = retnorm_var
        return {k: float(v) for k, v in metrics.items()}

    def _train_step(self, wm_state, actor_state, critic_state,
                    critic_target, retnorm_mean, retnorm_var, batch, rng_key):
        ...
        actor_state, critic_state, critic_target, retnorm_mean, retnorm_var, beh_metrics = (
            self._train_behavior(wm_state.params, actor_state, critic_state,
                                 critic_target, retnorm_mean, retnorm_var,
                                 states, k2)
        )

        metrics = {**wm_metrics, **beh_metrics}
        return wm_state, actor_state, critic_state, critic_target, retnorm_mean, retnorm_var, metrics
    ...
    def _train_world_model(self, wm_state, batch, rng_key):
        def wm_loss_fn(params):
            ...
            reward_logits = self.reward_pred.apply(params["reward"], flat_feat)
            reward_logits = reward_logits.reshape(B, T, self.cfg.twohot_bins)
            reward_loss = jnp.mean(self.twohot.loss(reward_logits, rewards))
    ...
    def _train_behavior(self, wm_params, actor_state, critic_state,
        ...
        def imagine(actor_params):
            ...
            for _ in range(cfg.imagination_horizon):
                ...
                r = self.twohot.pred(
                    self.reward_pred.apply(wm_params["reward"], feat_next))
                c = jax.nn.sigmoid(
        ...
        def actor_loss_fn(actor_params):
            ...
            values = self.twohot.pred(
                self.critic.apply(critic_target, feats))  # (B, H+1)
            returns = _lambda_return(rewards, values[:, 1:], conts, cfg.discount, cfg.lambda_)

            # Normalize returns for stable actor gradients
            rscale = jnp.maximum(jnp.sqrt(retnorm_var + 1e-8), 1.0)
            normed_returns = returns / rscale
            ...
            actor_loss = -normed_returns.mean() - cfg.entropy_scale * entropy
        ...
        # Critic loss (twohot on raw returns)
        def critic_loss_fn(critic_params):
            feats, rewards, conts = imagine(jax.lax.stop_gradient(actor_state.params))
            values = self.twohot.pred(
                self.critic.apply(critic_target, feats))
            returns = _lambda_return(rewards, values[:, 1:], conts, cfg.discount, cfg.lambda_)

            v_logits = self.critic.apply(critic_params, feats[:, :-1])
            critic_loss = jnp.mean(self.twohot.loss(
                v_logits, jax.lax.stop_gradient(returns)))
            return critic_loss, {"loss/value": critic_loss,
                                 "ret_raw": returns.mean()}
        ...
        raw_returns = critic_metrics["ret_raw"]
        # Use imag_return from actor as a proxy for batch stats
        batch_mean = actor_metrics["imag_return"]
        batch_var = jnp.maximum(raw_returns ** 2 - batch_mean ** 2, 1e-8)
        new_retnorm_mean = decay * retnorm_mean + (1 - decay) * batch_mean
        new_retnorm_var = decay * retnorm_var + (1 - decay) * batch_var
        ...
        del metrics["ret_raw"]  # internal only
        return actor_state, critic_state, critic_target, new_retnorm_mean, new_retnorm_var, metrics
