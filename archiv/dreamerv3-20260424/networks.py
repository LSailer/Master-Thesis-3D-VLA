...
class TwoHotDist:
    """Symexp two-hot distribution for scalar prediction (DreamerV3-style)."""

    def __init__(self, num_bins: int = 255, low: float = -20.0, high: float = 20.0):
        self.num_bins = num_bins
        self.bins = symlog(jnp.linspace(low, high, num_bins))

    def encode(self, target: jnp.ndarray) -> jnp.ndarray:
        """Two-hot encode a scalar target. target: (...)."""
        target_sl = symlog(target)
        below = jnp.sum(self.bins <= target_sl[..., None], axis=-1).astype(jnp.int32) - 1
        below = jnp.clip(below, 0, self.num_bins - 2)
        above = below + 1
        t = (target_sl - self.bins[below]) / (self.bins[above] - self.bins[below] + 1e-8)
        t = jnp.clip(t, 0.0, 1.0)
        return ((1 - t)[..., None] * jax.nn.one_hot(below, self.num_bins) +
                t[..., None] * jax.nn.one_hot(above, self.num_bins))

    def loss(self, logits: jnp.ndarray, target: jnp.ndarray) -> jnp.ndarray:
        """Cross-entropy loss. logits: (..., bins), target: (...)."""
        twohot = self.encode(target)
        log_probs = jax.nn.log_softmax(logits, axis=-1)
        return -(twohot * log_probs).sum(axis=-1)

    def pred(self, logits: jnp.ndarray) -> jnp.ndarray:
        """Expected value in original space. logits: (..., bins)."""
        probs = jax.nn.softmax(logits, axis=-1)
        return symexp(jnp.sum(probs * self.bins, axis=-1))
...
class Critic(nn.Module):
    ...
    @nn.compact
    def __call__(self, features: jnp.ndarray) -> jnp.ndarray:
        """Returns value logits (B, out_dim)."""
        return MLP(self.hidden, self.layers, self.out_dim, name="mlp")(features)
