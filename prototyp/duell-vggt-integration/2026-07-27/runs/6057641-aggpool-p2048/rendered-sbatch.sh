...
#SBATCH --output=output/runs/duell-l3-aggpool-p2048/slurm-%j.out
#SBATCH --error=output/runs/duell-l3-aggpool-p2048/slurm-%j.err

mkdir -p output/runs/duell-l3-aggpool-p2048

echo "Job $SLURM_JOB_ID on $(hostname) at $(date)"
echo "GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo 'N/A')"
GPU_MEMORY_LOG="output/runs/duell-l3-aggpool-p2048/gpu-memory-${SLURM_JOB_ID}.csv"
...
# Duell 2026-07-27 wave 1: thin variant of l3_aggregator_pooled for a 30-minute scored prod probe. Only prefill differs (2048 instead of the prod default 5000) to reach the replay gate (batch_size*seq_len=1024) faster and roughly double the trained steps inside the 30-minute window. See prototyp/duell-vggt-integration/2026-07-27/README.md.
uv run python scripts/r2dreamer/run.py habitat-l3-aggregator-pooled \
    --steps 1500000 \
    --prefill 2048 \
    --checkpoint_every 100000 \
    --output_dir "output/runs/duell-l3-aggpool-p2048/run-${SLURM_JOB_ID}" \
    --seed "${SEED}" \
    --log_every 250 \
    --buffer_capacity 500000 \
    --wandb_project 3d-vla-objectnav \
    --wandb_name "duell-l3-aggpool-p2048-s${SEED}-${SLURM_JOB_ID}" \
